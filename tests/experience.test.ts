...
test("a configuração principal contém uma história consistente", () => {
  ...
  assert.equal(EXPERIENCE.finale.emotionalDirection, "diversao-romantica-descontraida");
  assert.equal(EXPERIENCE.gift.favoriteFruit, "cereja");
  assert.equal(EXPERIENCE.playerTwo.callsPartner.includes("vida"), true);
  assert.equal(EXPERIENCE.relationship.originStory.length, 6);
  assert.equal(EXPERIENCE.relationship.officialProposal.date, "09/10/2025");
  assert.equal(EXPERIENCE.relationship.officialProposal.gifts.includes("aliança"), true);
