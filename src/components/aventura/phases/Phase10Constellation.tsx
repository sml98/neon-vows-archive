...
export function Phase10Constellation({ onNext, onBeep }: Phase10Props) {
  ...
  const handleCellClick = (r: number, c: number) => {
    ...
    const matchedWord = TARGET_WORDS.find(
      (w) => w === currentStr || w === currentStr.split("").reverse().join(""),
    );
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-md mx-auto space-y-6">
      <CyberTextBox>
        ...
        <p className="text-[var(--neon-cyan)] text-sm mb-4 text-center">NÍVEL: DECODIFICAÇÃO</p>
          ...
          <div className="grid grid-cols-8 gap-1 p-2 border-2 border-[var(--neon-cyan)] bg-black/80 rounded shadow-[0_0_15px_rgba(0,255,255,0.2)]">
            {GRID.map((row, r) =>
              row.split("").map((letter, c) => {
                ...
                let btnClass =
                  "w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center font-display text-lg sm:text-xl rounded transition-all ";

                if (isFound) {
                  btnClass +=
                    "bg-[var(--neon-pink)] text-black shadow-[0_0_10px_var(--neon-pink)] scale-105 z-10";
                } else if (isSelected) {
                  btnClass +=
                    "bg-[var(--neon-cyan)] text-black shadow-[0_0_10px_var(--neon-cyan)] scale-110 z-20";
                } else {
                  btnClass +=
                    "text-[var(--neon-cyan)] bg-black/50 border border-transparent hover:border-[var(--neon-cyan)]";
                }
